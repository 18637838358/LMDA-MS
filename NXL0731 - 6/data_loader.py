...
class BCICompetition4Set2A:
    ...
    def load(self):
        ...
        cnt.info["events"].clear()
        cnt.info["events"].append(events)
        cnt.info["temp"] = artifact_trial_mask
        return cnt
    ...
    def extract_data(self):
        ...
        # remember gdf events
        raw_gdf.info["events"].append(gdf_events[0])
        raw_gdf.info["events"].append(gdf_events[1])
        return raw_gdf

    def extract_events(self, raw_gdf):
        # all events
        events, name_to_code = raw_gdf.info["events"]
        ...
        if self.labels_filename is not None:
            mat_data = loadmat(self.labels_filename)
            classes = []
            for i in range(3,9):
                classes.extend(mat_data['data'][0][i][0][0][2].squeeze())
...
if __name__ == '__main__':
    ...
    train_label_filepath = train_filepath.replace(".gdf", ".mat").replace("BCICIV_2a_gdf", "mat")
    test_label_filepath = test_filepath.replace(".gdf", ".mat").replace("BCICIV_2a_gdf", "mat")

...
dataset_path = {'seed4': '/media/neaucs2/tc/dataset/BCICIV_2a_mat', 'seed3': '/media/neaucs2/tc/dataset/BCICIV_2a_mat'}
...
def get_data_label_frommat(mat_path, dataset_name, session_id):
    ...
    mat_de_data = mat_data['data'][0][session_id+3][0][0][0]
...
def get_allmats_name(dataset_name):
    ...
    for session in sessions:
        #if session != '.DS_Store':
            # mats = os.listdir(path + '/' + session)#'1'
            # mats.sort()
            # mats_list = []
            # for mat in mats:
            #     mats_list.append(mat)
            allmats.append(session)    
...
def load_data(dataset_name):
    ...
    da = np.zeros((45,3394,310))
    la = np.zeros((45,3394,1))
    for i in range(len(allmats)):
            s=allmats[i]
    #for i in allmats:
        #for j in range(len(allmats[0])):
            mat_path = path + '/' + s #str(i+1)+ '/' + allmats[i][j]
            one_data, one_label = get_data_label_frommat(
                mat_path, dataset_name, 0)
            da[i]=one_data
            la[i]=one_label
            #label[i][j] = one_label.copy()
    data=da.reshape(3,15,3394,-1,order='F')    
    label=la.reshape(3,15,3394,-1,order='F')    
    #for j in range(3):
        #for k in range(15):
            #data[j][k]=da0[j,k]
            #label[j][k]=la0[j,k]
